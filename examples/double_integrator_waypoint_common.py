"""Shared helpers for double-integrator waypoint training/evaluation scripts."""
...
class StateObservationAdapter(gym.Env):
    """Expose only the physical state to the policy while storing waypoint info."""

    metadata = {"render_modes": []}

    def __init__(self, **env_kwargs):
        super().__init__()
        self._base_env = DoubleIntegratorWaypointEnvV2(**env_kwargs)
    ...
    def _augment_info(self, info: Optional[Dict], obs: np.ndarray) -> Dict:
        ...
        data["target_waypoint"] = np.asarray(
            obs[self.nx : self.nx + 2], dtype=np.float32
        ).copy()
        return data
