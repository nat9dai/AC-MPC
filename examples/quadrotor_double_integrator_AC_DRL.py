"""Train and evaluate the AC-MPC stack on a 3-D double-integrator waypoint task."""
...
from ACMPC.envs import DoubleIntegrator3DWaypointEnv, build_velocity_dynamics_3d  # noqa: E402
...
from examples.quadrotor_double_integrator_waypoint_common import (  # noqa: E402
    build_env_manager,
    prepare_config,
    probe_dimensions,
)

DEFAULT_CONFIG_PATH = REPO_ROOT / "configs" / "quadrotor_double_integrator_waypoint.yaml"
DEFAULT_CHECKPOINT_DIR = REPO_ROOT / "checkpoints" / "quadrotor_double_integrator"
DEFAULT_EVAL_OUTPUT_DIR = REPO_ROOT / "eval_outputs" / "quadrotor_double_integrator"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--velocity-response", type=float, default=0.8)
    # Task
    parser.add_argument("--waypoint-range", type=float, default=1.0)
    parser.add_argument("--goal-radius", type=float, default=0.15)
    parser.add_argument("--min-start-radius", type=float, default=0.2)
    # Reward
...
def run_eval_episode(
    ...
    env: DoubleIntegrator3DWaypointEnv,
    *,
    history_window: int,
    max_steps: int,
    device: torch.device,
...
def evaluate_checkpoint(
    ...
    *,
    ...
    env = DoubleIntegrator3DWaypointEnv(**env_kwargs)
    try:
...
def main() -> None:
    ...
    # Switch to MLP actor/critic (must be set before prepare_config)
    ...
    vis_bounds = dict(xlim=[-2.0, 2.0], ylim=[-2.0, 2.0], zlim=[-2.0, 2.0])
    ...
    print("=== 3-D Double Integrator AC-MPC Training ===")
    ...
    print(
        ...
        f"Velocity response: {env_kwargs['velocity_response']}"
    )
            ...
            if args.visualize:
                try:
                    ...
                    def make_vis_env(env_id, **kwargs):
                        return DoubleIntegrator3DWaypointEnv(env_id=env_id, **kwargs)
